...
test("selectVoiceWindowSamples 在固定预算内为每个主要临时标签保留聚类所需证据", () => {
  const windows = [];
  ...
  const sampled = selectVoiceWindowSamples(windows, 16, { minPerLabel: 4 });
  assert.equal(sampled.length, 16);
  for (const label of ["说话人 1", "说话人 2", "说话人 3", "说话人 4"]) {
    assert.equal(sampled.filter((window) => window.legacySpeaker === label).length, 4, `${label} 应有 4 个样本`);
  }
