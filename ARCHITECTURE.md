...
4. **结束会议先封存，再补偿**：停止录音后先完成源音频写入；尾段文件校准在 70 秒绝对期限内排空正在运行的窗口及其递归尾窗，确认没有待提交中心区间后才允许收口。仅在超时或失败时把剩余草稿标记为 `realtime_fallback`；文件稿不得在兜底后继续写入同一时间区间。会后说话人重算是独立、有超时的后台补偿，不阻塞归档或纪要生成。
