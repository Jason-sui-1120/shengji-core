...
- **共享 core**（`shengji-core`）：53 个模块 + 31 组共享测试——纯业务逻辑（时间轴、稳定稿、去重、说话人轨道、尾段收口、纪要、待办）
- **两端 Adapter**：
...
|---|---|---|---|---|
| 实时预览 | 流式 ASR partial 到达 | 内存预览 | 顶部“当前正在识别”的一句话，可随上游回改 | 不进入时间轴列表，不能作为纪要、待办或最终事实来源 |
...
1. **SentenceEnd 是服务端落草稿的硬触发**：浏览器 `vad.endpoint` 丢失、连续发言或页面短暂断连，都不得让已确认句末长期只停留在顶部实时预览。草稿落库只做确定性规范化；词库异步预热，逐句 LLM、实时声纹、diarization 与分段 WAV 不得阻塞该关键路径。
2. **滚动校准只提交 45 秒中心区间**：前后上下文仅帮助断句和识别；落库前按词级时间、连续前后缀消重和单调时间约束处理。文件稿无法完整映射实时草稿时，整段中心区间以文件稿替换未人工编辑的自动行，不能把未映射残片留到尾段再混入成稿；替换时仅按时间轴继承已有说话人提示，不伪造新的声纹结论。
3. **自动分析以 stable revision 触发**：文件稿直接插入或替换均必须推进 revision 并通知分析；实时预览和纯草稿不触发会议结论。
4. **结束会议先封存，再补偿**：停止录音后先完成源音频写入；尾段文件校准在 70 秒绝对期限内排空正在运行的窗口及其递归尾窗，确认没有待提交中心区间后才允许收口。整个尾窗链（含会中队列转尾段、递归窗口和失败重试）必须共享同一个 deadline 取消信号；deadline 后返回的文件结果必须在任何数据库写入前丢弃。仅在超时或失败时把剩余草稿标记为 `realtime_fallback`；文件稿不得在兜底后继续写入同一时间区间。会后说话人重算是独立、有超时的后台补偿，不阻塞归档或纪要生成。
5. **人工编辑优先**：用户编辑的文字和说话人不被文件 ASR、说话人分离或重跑覆盖。

### 会议说话人轨道（唯一编号源）

实时声纹、45 秒文件分离和会后聚类都只提供“观察证据”，不得各自创建一套 `说话人 N`。三者统一调用 `meeting-speaker-track-manager.mjs`，由它维护会议级轨道：

1. **实时先显示、后台纠正**：草稿立即沿用当前暂定轨道，首条默认为 `说话人 1`；声纹异步返回后按同一 transcript id 原位更新，不能阻塞草稿落库，也不能新增重复行。
2. **弱证据不增加人数、不训练画像**：短句、噪声或无法与既有轨道清晰区分的声纹只用于即时匹配或累计为隐藏候选，不出现在说话人列表，也不改写已确认画像的质心、样本数和累计时长；只有 45 秒文件分离与会后完整录音聚类等强证据可以学习画像。
3. **45 秒局部编号不可直写**：分离模型的 `speaker_0/1` 只在当前窗口有效，必须提取各局部轨道声纹并映射到会议轨道后，才能异步回填稳定行。
4. **会后只校正、不重新编号、不阻塞归档**：完整录音聚类仍复用已经建立的会议轨道；它可以补齐和纠正自动行，但不从 `说话人 1` 另起一套编号。整行回填必须同时满足有效覆盖率和主导比例门槛，少量重叠或多人均分时保持待识别；任务在后台独立运行，成功后刷新快照与分析，失败不影响结束会议和首版纪要。
5. **只有一个降级方向**：声纹服务不可用时保持当前暂定轨道，等待下一次 45 秒或会后证据；禁止启用另一套本地编号器制造平行身份。

该实现不新增数据库表、后台服务或前端协议：继续复用 `speaker_profiles`、转写行的 speaker 字段及 `transcript.final` 按 id 替换语义。端侧只保留 SQLite/MySQL 存储差异。

### 分阶段实施与验收

| 阶段 | 范围 | 状态 | 验收门槛 |
|---|---|---|---|
| P0 | SentenceEnd 独立落草稿、稳定稿 revision 通知、部分对齐窗口整体替换、尾段收口状态兼容、双端配置注入 | 已实施，待线上 1 倍速回归 | 无 VAD endpoint 仍在约 300ms 后出现草稿；草稿不消失；稳定窗口不保留实时残片；首个稳定稿到达后实时总结更新；尾段失败仍可归档 |
| P1 | 前端时间轴单一数据源、实时声纹与会后重算完全后台化、草稿延迟可观测 | 已实施，待线上 1 倍速回归 | partial 只在顶部气泡；列表没有客户端预览、重复或错误置顶；说话人服务慢或不可用不增加草稿落库延迟；结束会议不等待会后说话人任务 |
